...
contract Weapon_Manifest {
    function idToWpn(uint256 _wpnID) public pure returns(dl._Weapon memory){
        if(_wpnID == 1){
            return baton();
        }else if(_wpnID == 2){
            return metalRod();
    ...
    function baton() public pure returns(dl._Weapon memory _weapon){
        ...
        _weapon.damage_type = "Slashing";
        _weapon.limit_type = false;
        _weapon.limit = 0;
        _weapon.ability = "none";
        _weapon.cost = 10;
    ...
    function metalRod() public pure returns(dl._Weapon memory _weapon){
        ...
        _weapon.damage_type = "Bludgeoning";
        _weapon.limit_type = false;
        _weapon.limit = 0;
        _weapon.ability = "none";
        _weapon.cost = 5;
